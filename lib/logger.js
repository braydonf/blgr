...
class Logger {
  ...
  /**
   * Truncate the log file to the last 20mb. Sycnronous, blocking.
   * @method
   * @private
   */
  ...
  truncateSync() {
    ...
    fs.readSync(fd, data, 0, maxSize, stat.size - maxSize);
    fs.ftruncateSync(fd, maxSize);
    fs.writeSync(fd, data, 0, maxSize, 0);
    fs.closeSync(fd);
    this._bytesSinceTruncate = 0;
  }
